...
function generateNickname(): string {
  return `${randomItem(ADJECTIVES)} ${randomItem(EXPLORERS)}`;
}
...
const selfNickname = document.getElementById('self-nickname') as HTMLElement | null;
...
function updateSelfNickname(): void {
  if (selfNickname) {
    selfNickname.textContent = state.nickname
      ? `You're playing as ${state.nickname}.`
      : '';
  }
}
...
function currentHashCode(): string | null {
  const hash = window.location.hash.replace(/^#/, '').trim();
  return hash.length > 0 ? hash.toUpperCase() : null;
}

function syncLocationHash(roomCode: string): void {
  const normalized = roomCode.toUpperCase();
  if (currentHashCode() === normalized) {
    return;
  }
...
function updateRoomStatus(snapshot: RoomSnapshot): void {
  const shareLink = buildInviteLink(snapshot.roomCode);
  if (roomCodeDisplay) {
    ...
    roomCodeDisplay.append(label, anchor);
  }
  if (startButton) {
  ...
  if (inviteHint) {
    inviteHint.textContent = state.isHost
      ? `Share this link with your friend: ${shareLink}`
      : `Share this page with your friend: ${shareLink}`;
  }
  syncLocationHash(snapshot.roomCode);
...
function setupSocket(): void {
  ...
  state.socket.on('connect', () => {
    attemptAutoJoinFromHash();
  });
}

function attemptAutoJoinFromHash(): void {
  if (state.roomCode || autoJoinTriggered) {
    return;
  }
  const roomCode = currentHashCode();
  if (!roomCode) {
    return;
  }
  autoJoinTriggered = true;
  if (inviteHint) {
    inviteHint.textContent = `Joining private session ${roomCode}…`;
  }
  state.socket.emit(
    'room:join',
    ...
    (response: JoinRoomResponse) => {
      if (!response.ok || !response.snapshot) {
        ...
        if (inviteHint) {
          inviteHint.textContent = response.message ?? 'Unable to join room';
        }
        return;
      }
